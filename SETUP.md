...
- (Optional, host machine) If you enabled Grok as a DM engine (§7), you can
...
## 7. (Optional) Enable Grok — for images and/or the DM engine

The `grok` CLI powers two independent, optional features; **neither is
required to play** (the default DM engine is Claude, and images are off by
default):
...
- **Grok as the DM engine** — you can run a campaign's DM on Grok instead of
  Claude, chosen per campaign (ADR-0018).

Both need the same one-time CLI setup:
...
2. For images: in the app, open **Settings → The Look** and turn on
   **Generate scene art** (optionally pick an art style).
3. To run the DM on Grok: open **Settings → The Engine** (or the New
   Chronicle screen) and switch the engine toggle from **Claude** to
   **Grok**, then pick a Grok model. Provider and model are per-campaign and
   switching either resets that campaign's DM session (ADR-0018).
